...
\begin{multicols}{3}
	...
	\begin{mybox}{accent}{}{enhanced, watermark tikz={\draw[line width=2mm] circle (1cm)
		...
		\centering
		\savebox0{\bigjava{int[] arr}}
		\begin{tikzpicture}
			\node [coolgraphnode] (fa) [] {\usebox0};
			...
			\draw [arrow, opacity=0.3] (lol) to [out=5, in=-5] node [midway, below, sloped, color=darkgray, opacity=0.3] {read from} (fa);
		\end{tikzpicture}
	\end{mybox}%
	\vspace*\fill%
\end{multicols}
...
\begin{multicols}{4}
	...
	\begin{enumerate*}
		...
		\item assignments,
	\end{enumerate*}
	as they influence the data flow.
	\par
	...
	types: \emph{Def}, if the element is a declaration or on the left hand side of an
	assignment and \emph{Use}, otherwise.
	\par
	Now we can hook up the newly added node. For this purpose we get all recently added
	elements that \emph{share a declaration} with the current element. We then connect
	the newly added node to all recently added nodes, only \emph{omitting Use -> Use}
	edges.
	\par
	...
	elements in the assignment. This is done by getting all active elements from the
	right hand side and drawing an edge from the \emph{left hand side to them}.
	\par
	The types of two nodes alone lets us infer the type of the edge connecting them. We
	distinguish between three edge types: \emph{Read From}
	(\javainline{Use}~$\rightarrow$~\javainline{Def}), \emph{Defined By}
	(\javainline{Def}~$\rightarrow$~\javainline{Use}), and \emph{Occurrence}
	(\javainline{Def}~$\rightarrow$~\javainline{Def}).
	\par
	Getting the data flow to, from, or between elements can now be narrowed down to a
	\emph{graph reachability problem}.
	\begin{description}
		\item [to] We return all elements that can reach the given element.
		\item [from] We return all elements that can reached by the given element.
		\item [between] We return all paths between two elements.
	\end{description}
	\vspace*\fill
	\columnbreak
	...
	\begin{minipage}{\dimexpr2\columnwidth+\columnsep}
		...
		\begin{mybox}{accent}{}{sidebyside, sidebyside align=center}
			...
			\begin{tikzpicture}
				...
				\node [coolgraphnode] (ao) [right=of at, xshift=1.5cm] {\bigjava{|\textbf{a}| = n + 5}};

				\node [coolgraphnode] (n) [above=of ao] {\bigjava{|\phantom{i}\llap{a}| = |\textbf{n}| + 5}};
				...
				\draw [arrow] (ae) to [in=0, out=0] node [midway, sloped, below, color=darkgray] {occurrence} (ao);
		...
		\section*{Future Work}
		While basic cases are already handled well, a variety of situations are handled
		poorly or not at all. Here are some areas we want to improve in:
		\begin{description}
			...
			\item [Recursion] If a method calls itself, either directly or indirectly,
				we need to recognize this when generating and traversing the data flow
				graph.
			\item [Arrays] We currently treat arrays as a single \oldemph{thing}. But
	...
	\par
	The current AlDeSCo-Prototype uses \emph{Spoon} to generate an AST for any given
	Java source code. We then traverse Spoon's AST in order to build the data flow
	graph, which is represented by \emph{JGraphT}'s data structures. The actual spanning
	of the data flow graph is completely done by ourselves without the help of a
	library.
